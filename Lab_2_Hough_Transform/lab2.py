...
def make_gaussian_kernel(ksize, sigma):
    ...
    # construct x and y indices
    x = np.tile(np.arange(ksize).reshape(-1,1), (1,ksize))
    y = np.tile(np.arange(ksize).reshape(1,-1), (ksize,1))

    # create gaussian kernel
    kernel = np.exp((np.square(x-x_mean) + np.square(y-y_mean))/(-2 * np.square(sigma)))
...
def estimate_gradients(original_img, display=True):
    ...
    d_mag = np.sqrt(np.square(dx) + np.square(dy))
...
def non_maximum_suppression(d_mag, d_angle, display=True):
    ...
    # return the appropriate neighbor offset based on angle
    def angle_to_neigh(angle_deg):
        angle_deg = (angle_deg + 180) % 180
        if 157.5 < angle_deg <= 180 or 0 < angle_deg < 22.5:
            return (1, 0)
    ...
    # loop through the whole d_mag
    x_dim, y_dim = d_mag.shape
    for x in range(x_dim):
        for y in range(y_dim):
            larger_than_neigh = True
            x_neigh_off, y_neigh_off = angle_to_neigh(d_angle_180[x,y])
            # compare current d_mag with each of the 2 neighbors
            for sign in (1, -1):
                x_neigh = x + sign*x_neigh_off
                y_neigh = y + sign*y_neigh_off
                if 0 <= x_neigh < x_dim and 0 <= y_neigh < y_dim:
                    if d_mag[x,y] <= d_mag[x_neigh, y_neigh]:
                        larger_than_neigh = False
                        break
            # if current d_mag larger then both neighbors, set d_mag in output, otherwise output remain zero
            if larger_than_neigh:
                out[x,y] = d_mag[x,y]
...
def non_maximum_suppression_interpol(d_mag, d_angle, display=True):
    ...
    # return the appropriate neighbors offset and their interpolation ratio based on angle
    def angle_to_neigh_interpol(angle_deg):
        angle_deg = (angle_deg + 180) % 180
        if 0 < angle_deg <= 45:
            return (1, 1), (1, 0), np.tan(np.deg2rad(angle_deg-0))
        elif 45 < angle_deg <= 90:
            return (0, 1), (1, 1), np.tan(np.deg2rad(angle_deg-45))
        elif 90 < angle_deg <= 135:
            return (-1, 1), (0, 1), np.tan(np.deg2rad(angle_deg-90))
        else:
            return (-1, 0), (-1, 1), np.tan(np.deg2rad(angle_deg-135))
    ...
    for x in range(x_dim):
        for y in range(y_dim):
            larger_than_neigh = True
            neigh_off_1, neigh_off_2, interpol_ratio = angle_to_neigh_interpol(d_angle_180[x,y])
            # compare current d_mag with each of the 2 neighbors
            for sign in (1, -1):
                x_neigh_1 = x + sign*neigh_off_1[0]
                y_neigh_1 = y + sign*neigh_off_1[1]
                x_neigh_2 = x + sign*neigh_off_2[0]
                y_neigh_2 = y + sign*neigh_off_2[1]
                if (0 <= x_neigh_1 < x_dim and 0 <= y_neigh_1 < y_dim and 0 <= x_neigh_2 < x_dim and 0 <= y_neigh_2 < y_dim):
                    # neighbor value is acquired by utilizing interpolation
                    if d_mag[x,y] <= interpol_ratio*d_mag[x_neigh_1, y_neigh_1] + (1-interpol_ratio)*d_mag[x_neigh_2, y_neigh_2]:
                        larger_than_neigh = False
                        break
...
def double_thresholding(inp, perc_weak=0.1, perc_strong=0.3, display=True):
    ...
    weak_edges = np.logical_and(low_threshold <= inp, inp < high_threshold)
    # END
...
def edge_linking(weak, strong, n=200, display=True):
    ...
    strong_x_dim = strong.shape[0]
    strong_y_dim = strong.shape[1]
    
    # loop a fixed number of pass, early stopping if no more changes
    for _ in range(n):
        # construct a padded strong array
        padded_strong = np.zeros((strong_x_dim+2, strong_y_dim+2), dtype=strong.dtype)
        padded_strong[1:strong_x_dim+1, 1:strong_y_dim+1] = strong

        # construct the 8 channel arrays of offset
        offset_arrays = np.zeros((strong_x_dim, strong_y_dim, 8), dtype=strong.dtype)
        c = 0
        for off_x in (-1, 0, 1):
            for off_y in (-1, 0, 1):
                if off_x == 0 and off_y == 0:
                    continue
                offset_arrays[:,:,c] = padded_strong[1+off_x:strong_x_dim+1+off_x, 1+off_y:strong_y_dim+1+off_y]
                c += 1

        # sum thorugh the whole 8 channels, strong pixel found when sum is more than 0
        offset_arrays_sum = np.sum(offset_arrays, axis=2)
        has_strong_neigh = offset_arrays_sum > 0

        weak_but_has_strong_neigh = np.logical_and(weak, has_strong_neigh)

        # early stopping if no new strong pixel found
        if np.sum(weak_but_has_strong_neigh) == 0:
            break

        # update strong and weak accordingly
        strong[weak_but_has_strong_neigh] = 1
        weak[weak_but_has_strong_neigh] = 0
...
# 1/2/3 IMPLEMENT
def hough_vote_lines(img, interval_rho=1, interval_theta=np.pi/180):
    '''
    ...
    max_rho = np.hypot(*img.shape[:2])
    min_rho = -max_rho
    min_theta, max_theta = 0, np.pi
    ...
    distances = np.linspace(min_rho, max_rho, num=dim_rho, endpoint=False)
    thetas = np.linspace(min_theta, max_theta, num=dim_theta, endpoint=False)
    ...
    #         idx_theta = int((theta-min_theta)/interval_theta)
    #         idx_rho = int((rho-min_rho)/interval_rho)

    #         A[idx_rho,idx_theta] += 1

    # Array version (fast)
    xs, ys = np.nonzero(img)
    rhos = np.multiply.outer(xs, np.cos(thetas)).flatten() + np.multiply.outer(ys, np.sin(thetas)).flatten()
    idx_thetas = np.tile(np.arange(dim_theta), len(xs))
    idx_rhos = ((rhos-min_rho)/interval_rho).astype(int)

    idxs = np.array(list(zip(idx_rhos, idx_thetas)), dtype="i,i")
    uniq_idxs, count = np.unique(idxs, return_counts=True)
    uniq_idxs_x, uniq_idxs_y = zip(*uniq_idxs)
    A[uniq_idxs_x, uniq_idxs_y] = count
...
def hough_vote_circles(img, radius = None, weighted_voting=False):
    '''
    ...
    R_dim = int((R_max-R_min)/R_interval)
    X_dim = int((X_max-X_min)/X_interval)
    Y_dim = int((Y_max-Y_min)/Y_interval)

    A = np.zeros((R_dim, X_dim, Y_dim))
    R = np.linspace(R_min, R_max, num=R_dim, endpoint=False, dtype=int)
    X = np.linspace(X_min, X_max, num=X_dim, endpoint=False, dtype=int)
    Y = np.linspace(Y_min, Y_max, num=Y_dim, endpoint=False, dtype=int)

    # Loop version
    for x, y in np.argwhere(img):
        for r in R:
            votes_x, votes_y = circle_perimeter(x, y, r, shape=img.shape)
            votes_x = ((votes_x - X_min) / X_interval).astype(int)
            votes_y = ((votes_y - Y_min) / Y_interval).astype(int)
            vote_r = int((r - R_min)/R_interval)
            vote = 1
            if weighted_voting:
                vote = 1/r   # weight the vote inversely proportional to the radius
            A[vote_r,votes_x,votes_y] += vote
...
# IMPLEMENT
def hough_vote_circles_grad(img, d_angle, radius = None, interval_radius=1, interval_x=1, interval_y=1):
    '''
    Use the edge image to vote for 3 parameters: circle radius and circle center coordinates.
    We also accept a range of radii to save computation costs. If the radius range is not given, it is default to
    ...
    A = np.zeros((R_dim, X_dim, Y_dim), dtype=int)
    R = np.linspace(R_min, R_max, num=R_dim, endpoint=False, dtype=int)
    X = np.linspace(X_min, X_max, num=X_dim, endpoint=False, dtype=int)
    Y = np.linspace(Y_min, Y_max, num=Y_dim, endpoint=False, dtype=int)

    d_angle_cos = np.cos(d_angle)
    d_angle_sin = np.sin(d_angle)

    # Loop version
    for x, y in np.argwhere(img):
        for r in R:
            angle_cos = d_angle_cos[x,y]
            angle_sin = d_angle_sin[x,y]
            vote1_x = int((x + r*angle_cos - X_min) / X_interval)
            vote1_y = int((y + r*angle_sin - Y_min) / Y_interval)
            vote2_x = int((x - r*angle_cos - X_min) / X_interval)
            vote2_y = int((y - r*angle_sin - Y_min) / Y_interval)
            vote_r = int((r - R_min) / R_interval)
            if 0 <= vote1_x < X_dim and 0 <= vote1_y < Y_dim:
                A[vote_r,vote1_x,vote1_y] += 1
            if 0 <= vote2_x < X_dim and 0 <= vote2_y < Y_dim:
                A[vote_r,vote2_x,vote2_y] += 1
